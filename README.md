...
- `rss` — RSS / Atom 피드
- `bi_pusan` — 창업지원단(bi.pusan.ac.kr). Vue 로 만들어져 HTML 에 글이 없어서 내부 API 를 직접 호출합니다

> 요즘 사이트는 화면이 뜬 뒤 자바스크립트로 목록을 채우는 경우가 많습니다.
> 그런 사이트는 `generic` 으로 안 되고, 창업지원단처럼 전용 파서가 필요합니다.
> `add` 가 "표를 찾지 못했습니다" 라고 하면 대개 이 경우입니다.
